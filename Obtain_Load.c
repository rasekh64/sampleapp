...
DEFINE_ON_DEMAND(get_load_beam){
	...
	int n_rings;
	Ring *CL_pos;
	int surfaceID = 27; /*Surface ID of the surface to retrieve the load, could use an array as in Tango and distinguish for end face */
#if !RP_HOST
	Thread *face_thread;
	face_t face;
	...
	FILE *file_load;
	char filename_CL[256];
	FILE *file_CL;
	int size_bis;
	real dist;
#endif /* !RP_NODE */

/* Read the file containing the current centerline position required for converting to load per unit length later on */
#if !RP_NODE
	sprintf(filename_CL, "CL_Old.dat"); /* Should contain the current configuration of the centerline */
	file_CL = NULL;
	if ((file_CL = fopen(filename_CL, "r")) == NULL) {
		Error("\nUDF-error: Unable to open %s for writing\n", filename_CL);
		exit(1);
	}

	fscanf(file_CL, "%i", &n_rings); 	/* Read the first line which contains the number of node rings */
	/*printf("n_rings = %i",n_rings);fflush(stdout);*/
	ASSIGN_MEMORY(CL_pos, n_rings, Ring);
	for (i=0; i < n_rings; i++) {
		/* Here I also initialize the ring id assuming that the files are ordered according to ring id */
		CL_pos[i].id = i;
		for (j = 0; j < ND_ND; j++)
			fscanf(file_CL, "%lf", &(CL_pos[i].coord[j]));
		}
	fclose(file_CL);
#endif /* !RP_NODE */
...
/* Write header for the files */
#if RP_2D
		fprintf(file_load, "%27s%27s%27s\t[N/m]\n","0-column","x-force","y-force");
#else /* RP_2D */
		fprintf(file_load, "%27s%27s%27s%27s\t[N/m]\n","0-column","x-force","y-force","z-force");
#endif /* RP_2D */
#endif /* !RP_NODE */
	...
	host_to_node_int_1(n_rings);
		...
		for (i = 0; i < size_bis/(ND_ND+1); i++) {
			...
			dist = sqrt(pow(CL_pos[i+1].coord[0]-CL_pos[i].coord[0],2)+pow(CL_pos[i+1].coord[1]-CL_pos[i].coord[1],2)+pow(CL_pos[i+1].coord[2]-CL_pos[i].coord[2],2));
			/*printf("dist = %f \n",dist);fflush(stdout);*/
			for (j = 0; j < ND_ND; j++)
				fprintf(file_load, "%27.17E", array_sum[(ND_ND+1)*i+j]/dist);
			fprintf(file_load, "%27.17E\n", array_sum[(ND_ND+1)*i+ND_ND]/dist);
		}
#endif /* RP_NODE */
#else /* PARALLEL */
		...
		for (i = 0; i < size/(ND_ND+1); i++) {
			...
			/*printf("dist = %f \n",dist);fflush(stdout);*/
			for (j = 0; j < ND_ND; j++)
				fprintf(file_load, "%27.17E", array[(ND_ND+1)*i+j]/dist);
			fprintf(file_load, "%27.17E\n", array[(ND_ND+1)*i+ND_ND]/dist);
		}
